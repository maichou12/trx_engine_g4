...
public class UserService {
    ...
    public ApiResponse registerUser(RegisterRequest registerRequest) {
        ...
        try {
            ...
            if (phoneExists(registerRequest.getTelephone())) {
                return new ApiResponse("Ce numéro de téléphone est déjà enregistré", 409,false );
            }
            ...
            ApiResponse usernameCheck = keycloakService.usernameExists(username);

            if (!usernameCheck.isSuccess() && usernameCheck.getStatusCode() == 409) {
                log.warn("❌ Username '{}' déjà pris.", username);
                return new ApiResponse("Le nom d'utilisateur '" + username + "' est déjà pris ou indisponible.", 409,false);
            }
            ...
            if (!usernameCheck.isSuccess() && usernameCheck.getStatusCode() >= 500) {
                ...
                return new ApiResponse("Erreur de connexion au serveur d'identité Keycloak (vérifiez les logs KeycloakService).", usernameCheck.getStatusCode(), false);
            }
            ...
            if (e instanceof ConstraintViolationException) {
                return new ApiResponse("Validation DB locale échouée: Assurez-vous que le NIN est présent et le téléphone valide.", 400,false);
            }
    ...
    private ApiResponse validateRegistrationData(RegisterRequest request) {
        ...
        if (request.getTelephone() == null || request.getTelephone().trim().isEmpty()) {
            return new ApiResponse("Le numéro de téléphone est obligatoire",400, false);
        }
        ...
        if (request.getPrenom() == null || request.getPrenom().trim().isEmpty()) {
            return new ApiResponse("Le prénom est obligatoire",400, false);
        }

        // Vérification nom
        if (request.getNom() == null || request.getNom().trim().isEmpty()) {
            return new ApiResponse("Le nom est obligatoire", 400,false);
        }

        // AJOUT : Validation du nom d'utilisateur
        if (request.getNomUtilisateur() == null || request.getNomUtilisateur().trim().isEmpty()) {
            return new ApiResponse("Le nom d'utilisateur est obligatoire", 400,false);
        }

        // Optionnel : Vérifier qu'il n'y a pas d'espaces ou caractères spéciaux
        if (!request.getNomUtilisateur().matches("^[a-zA-Z0-9._-]+$")) {
            return new ApiResponse("Le nom d'utilisateur ne doit contenir que des lettres, chiffres, points ou tirets.", 400,false);
        }

        return new ApiResponse("Validation réussie", 200,true );
    }
...
//        userDTO.setEmail(request.getEmail());
        ...
        userDTO.setPassword(request.getPassword()); // Ajouter le mot de passe pour Keycloak
        return userDTO;
    }
     ...
     */
    public ApiResponse getUserByPhone(String telephone) {
        try {
            Optional<User> user = userRepository.findByTelephone(telephone);
            if (user.isPresent()) {
                UserDto userDto = modelMapper.map(user.get(), UserDto.class);
                return new ApiResponse("Utilisateur trouvé", 200, userDto);
            } else {
                return new ApiResponse("Aucun utilisateur avec ce numéro de téléphone", 404, false);
            }
        } catch (Exception e) {
            return new ApiResponse("Erreur lors de la recherche: " + e.getMessage(), 500, false);
        }
    }

    // Dans UserService.java, changez le retour de saveUserInDatabase
    public ApiResponse saveUserInDatabase(UserDto userDTO, String keycloakUserId) { // ⬅️ CHANGEMENT
        ...
        try {
            ...
            return new ApiResponse("Utilisateur enregistré avec succès.", 200, savedUser); // ⬅️ CHANGEMENT

        } catch (Exception e) {
            log.error("❌ Erreur DB : {}", e.getMessage(), e);
            return new ApiResponse("Erreur DB : " + e.getMessage(),500, false);
        }
    }
    ...
    public ApiResponse addUser(UserDto userDTO) {
        try {
            ...
            ApiResponse usernameCheck = keycloakService.usernameExists(userDTO.getNomUtilisateur());
            if (!usernameCheck.isSuccess()) {
                return new ApiResponse("Nom d'utilisateur déjà pris : " + userDTO.getNomUtilisateur(),  409, false);
            }

            // Vérification si le téléphone existe déjà
            if (phoneExists(userDTO.getTelephone())) {
                return new ApiResponse("Numéro de téléphone déjà enregistré : " + userDTO.getTelephone(), 409, false);
            }

            // Vérification si le rôle existe dans Keycloak
            Boolean roleCheck = keycloakService.roleExists(userDTO.getRoleName());
            if (!roleCheck) {
                return new ApiResponse("Le rôle spécifié n'existe pas dans Keycloak : " + userDTO.getRoleName(), 400,false);
            }
            ...
            try {
                ...
                if (response.isSuccess() && keycloakUserId != null) {
                    ApiResponse saveResponse = saveUserInDatabase(userDTO, keycloakUserId);
                    if (saveResponse.isSuccess()) {
                        return new ApiResponse("Utilisateur créé avec succès dans Keycloak et dans la base de données locale",  201, null);
                    } else {
                        return saveResponse;
                    }
                } else {
                    return new ApiResponse("Erreur lors de l'ajout du rôle à l'utilisateur dans Keycloak",  500, false);
                }

            } catch (Exception e) {
                return new ApiResponse("Erreur lors de la création de l'utilisateur dans Keycloak" + e.getMessage(),  500, false);
            }

        } catch (Exception e) {
            return new ApiResponse("Erreur lors de la création de l'utilisateur : " + e.getMessage(),  500, false);
        }
    }
    ...
    public ApiResponse updateUser(String userId, UserDto userDTO) {
        try {
            ...
            if (!optionalUser.isPresent()) {
                return new ApiResponse("Utilisateur non trouvé avec l'ID : " + userId,  404, false);
            }
            ...
            if (keycloakUpdateResponse.isSuccess()) {
                ...
                return new ApiResponse("Utilisateur mis à jour avec succès",  200, true);
            } else {
                return new ApiResponse("Erreur lors de la mise à jour dans Keycloak",  500, false);
            }
        } catch (Exception e) {
            return new ApiResponse("Erreur lors de la mise à jour de l'utilisateur : " + e.getMessage(),  500, false);
        }
    }
    ...
    public ApiResponse getAllUsers() {
        try {
            ...
            return new ApiResponse("Utilisateurs récupérés avec succès", 200, users);
        } catch (Exception e) {
            return new ApiResponse("Erreur lors de la récupération des utilisateurs: " + e.getMessage(),  500, false);
        }
    }
...
//                            "numCompte", compteResponse.getData()
            ...
            // Gestion des erreurs Keycloak non catchées ou autres erreurs inattendues
            return new ApiResponse("Erreur technique: " + e.getMessage(),  500, false);
        }
    }
